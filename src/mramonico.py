...
obras = './obras.json'
obras = os.path.join(os.path.dirname(__file__), obras)
...
def load_frames(json_file):
    ...
    for frame in west:
        ...
        antifr = Frame(img="assets/" + frame["file"],
                       ...
                       scale_x = frame["scale"],
                       description = frame["description"])
        antifr.rotation_y += frame["Yrotate"] + 90
        antifr.rotation_x += frame["Xrotate"] - 185 
...
class Spectator(FirstPersonController):
    def __init__(self):
        super().__init__(position = (0, 5, 0))
        self.flying = False
    
    def update(self):
        if self.flying:
            self.gravity = 0
            self.y += (held_keys["v"] - held_keys["b"]) * time.dt * 30
        else:
            self.gravity = 1
